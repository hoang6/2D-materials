...
void molInfo(const AMod::Molecule& _mol, 
	     double _h[9], double& _bondr, double& _Dz);
...
int main(int argc, char* argv[]) {
  ...
  cout << "Dz_mean    " << data_mean[10] << endl;
  cout << "Dz_var     " << data_var[10] << endl;
...
void molInfo(const AMod::Molecule& _mol, 
  ...
  for(k = 0; k < natoms; k++) 
    _Dz += Util::square(_mol.atom(k)->x[2]-mc[2]);
  _Dz = std::sqrt(_Dz/(natoms-1));
}
