...
int main(int argc, char* argv[]) {
  ...
  if(pmc == NULL) {
    cout << "runMCNPH: unrecognized potential: " << potTag << endl;
    return 1;
  }  

  pmcAgent = MC::allocMCAgent(agentTag, cin);
  if(pmcAgent == NULL) {
    cout << "runMCNPH: unrecognized MCAgent: " << agentTag << endl;
    return 2;
  }
  ...
  if(cin.fail()) {
    cout << "runMCNPH: illegal parameters" << endl;
    return 3;
  }
  cin >> mol;
  if(cin.fail() || mol.natoms() == 0) {
    cout << "runMCNPH: no (initial) molecule is read" << endl;
    return 4;
  }
  if(mol.naxes() != 3) {
    cout << "runMCNPH: molecule must be in a 3D box" << endl;
    return 5;
  }
  ...
  MC::useNPH(seed,mol,*pmc,*pmcAgent,pressure,enthalpy,
