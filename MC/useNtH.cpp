...
namespace MC {
	...
	static void compute_te(const double& pressure,
		const double t[9],
		const double& volume0,
		const double inv_h0[9],
		const double h[9],
		double& out_v,
		double out_e[9],
		double out_te[9],
		...
		void useNtH(long seed,
			...
			Stream& stream) {
				...
				h_acc_ratio = 0.0;
				nStep = rStep+eStep;
				Util::mcopy(tension, t, 3, 3);
				Util::minverse(h0, inv_h0, 3);
				mol.axes2array(h);
				compute_te(pressure, t, volume0, inv_h0, h,
										old_v, e, te, tr_te, old_work);
										reset_flag = true;
										reset_n = 0;
				...
				if(stream.showHeader()) {
					...
					stream.stat() << "t" << " ";
					for(int i = 0; i < 9; i++)
					stream.stat() << std::showpos << tension[i] << " ";
					stream.stat() << std::endl;
					stream.stat() << "H" << " " << std::showpos << enthalpy << std::endl;
					stream.stat() << "V0" << " " << std::noshowpos << volume0 << std::endl;
					stream.stat() << "h0" << " ";
					for(int i = 0; i < 9; i++)
					stream.stat() << std::showpos << h0[i] << " ";
					stream.stat() << std::endl;
				...
				for(kStep = 0; kStep < nStep; kStep++) {
					for(kkStep = 0; kkStep < natoms; kkStep++) {
						...
						if(hStep*natoms*Util::RNG::uniform_SFMT() >= 1.0) {
							...
							//Metropolis MC
							old_K = enthalpy-(dEt+oE)-old_work;
							new_K = enthalpy-(dE+dEt+oE)-old_work;
							if(old_K <= 0.0 || new_K <= 0.0) {
								accept = false;
							}
							else {
								logw = (3.0*natoms/2.0-1.0)*(std::log(new_K/old_K));
								accept = (logw>=0.0 || logw>=std::log(1.0-Util::RNG::uniform_SFMT()));
							}
							if(accept) {//accept
								dEt += dE;
								acc_count++;
						...
						/********** TRIAL MOVE BOX **********/
							...
							try {
								...
								mol.axes2array(h);
								compute_te(pressure, t, volume0, inv_h0, h,
									new_v, e, te, tr_te, new_work);
									old_K = enthalpy-(dEt+oE)-old_work;
									new_K = enthalpy-(dE+dEt+oE)-new_work;
									if(old_K <= 0.0 || new_K <= 0.0) {
										accept = false;
									}
									else {
										logw =
										+natoms*(std::log(new_v/old_v))
										+(3.0*natoms/2.0-1.0)*(std::log(new_K/old_K));
										accept = (logw>=0.0 ||
											logw>=std::log(1.0-Util::RNG::uniform_SFMT()));
										}
									}
									catch(...) {
								...
								if(h_tot_count == 100) {
									...
									dhmax *= (1.0+h_acc_ratio-0.5);
										...
										if((kStep+1)%stream.dStatStep() == 0) {
											...
											stream.stat() << std::showpos << (enthalpy-oE-old_work) << " ";
										...
										void compute_te(const double& pressure,
											const double t[9],
											const double& volume0,
											const double inv_h0[9],
											const double h[9],
											...
											double& out_work) {
												double mtmp[9];
												double mtmp_tr[9];
												out_v = Util::abs(Util::cross_dot3(h, h+3, h+6));
												Util::mdotm(h, inv_h0, mtmp, 3, 3, 3);
												Util::mtrans(mtmp, mtmp_tr, 3, 3);
												Util::mdotm(mtmp_tr, mtmp, out_e, 3, 3, 3);
												for(int i = 0; i < 3; i++) out_e[4*i] -= 1.0;
												Util::kdotm(0.5, out_e, out_e, 3, 3);
												Util::mdotm(t, out_e, out_te, 3, 3, 3);
												out_tr_te = Util::mtrace(out_te, 3);
												out_work = pressure*out_v+volume0*out_tr_te;
											}
